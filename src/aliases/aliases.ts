export const aliases = {
  "oddType": {
    "underOver_0.5": { betfair: "Più/Meno", williamhill: "Over/Under 0.5 Goal" },
    "underOver_1.5": { betfair: "Più/Meno", williamhill: "Over/Under 1.5 Goal" },
    "underOver_2.5": { betfair: "Più/Meno", williamhill: "Over/Under 2.5 Goal" },
    "underOver_3.5": { betfair: "Più/Meno", williamhill: "Over/Under 3.5 Goal" },
    "underOver_4.5": { betfair: "Più/Meno", williamhill: "Over/Under 4.5 Goal" },
    "underOver_5.5": { betfair: "Più/Meno", williamhill: "Over/Under 5.5 Goal" },
    "underOver_6.5": { betfair: "Più/Meno", williamhill: "Over/Under 6.5 Goal" },
    "underOver_7.5": { betfair: "Più/Meno", williamhill: "Over/Under 7.5 Goal" },
    "rimborso pareggio": {
      betfair: "Rimborso in Caso di Pareggio"
    },
    "goal_yesNo": {
      betfair: "Mercati Segnano entrambe le squadre",
      williamhill: "Segneranno Entrambe le Squadre?"
    },
    'handicapCorners_["-1","+2"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    'handicapCorners_["-2","+3"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    'handicapCorners_["-3","+4"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    'handicapCorners_["-4","+5"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    'handicapCorners_["+2","-1"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    'handicapCorners_["+3","-2"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    'handicapCorners_["+4","-3"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    'handicapCorners_["+5","-4"]': { betfair: "Handicap calci d'angolo", williamhill: "Corner con Handicap" },
    "rigore_yesNo": { betfair: "Rigore Si/No", williamhill: "Rigore Si/No" },
  },
  "tournament": {
    "serie A": {
      betfair: "Serie A",
      williamhill: "Serie A"
    }
  },
  "role": {
    "-0.5": { williamhill: "Under 0.5", betfair: "Under 0.5" },
    "-1.5": { williamhill: "Under 1.5", betfair: "Under 1.5" },
    "-2.5": { williamhill: "Under 2.5", betfair: "Under 2.5" },
    "-3.5": { williamhill: "Under 3.5", betfair: "Under 3.5" },
    "-4.5": { williamhill: "Under 4.5", betfair: "Under 4.5" },
    "-5.5": { williamhill: "Under 5.5", betfair: "Under 5.5" },
    "-6.5": { williamhill: "Under 6.5", betfair: "Under 6.5" },
    "-7.5": { williamhill: "Under 7.5", betfair: "Under 7.5" },
    "+0.5": { williamhill: "Over 0.5", betfair: "Over 0.5" },
    "+1.5": { williamhill: "Over 1.5", betfair: "Over 1.5" },
    "+2.5": { williamhill: "Over 2.5", betfair: "Over 2.5" },
    "+3.5": { williamhill: "Over 3.5", betfair: "Over 3.5" },
    "+4.5": { williamhill: "Over 4.5", betfair: "Over 4.5" },
    "+5.5": { williamhill: "Over 5.5", betfair: "Over 5.5" },
    "+6.5": { williamhill: "Over 6.5", betfair: "Over 6.5" },
    "+7.5": { williamhill: "Over 7.5", betfair: "Over 7.5" },
    "yes": { betfair: "Sì", williamhill: "Si" },
    "no": { betfair: "No", williamhill: "No" }
  },
  "dayWord": {
    "sabato": {
      betfair: "sabato",
      williamhill: "sabato"
    },
    "domenica": {
      betfair: "domenica",
      williamhill: "domenica"
    },
    "lunedì": {
      betfair: "lunedì",
      williamhill: "lunedì"
    },
    "martedì": {
      betfair: "martedì",
      williamhill: "martedì"
    },
    "mercoledì": {
      betfair: "mercoledì",
      williamhill: "mercoledì"
    },
    "giovedì": {
      betfair: "giovedì",
      williamhill: "giovedì"
    },
    "venerdì": {
      betfair: "venerdì",
      williamhill: "venerdì"
    }
  },
  "monthWord": {
    "settembre": {
      betfair: "set",
      williamhill: "Set"
    },
    "febbraio": {
      betfair: "feb",
      williamhill: "Feb"
    },
    "marzo": {
      betfair: "mar",
      williamhill: "Mar"
    },
    "aprile": {
      betfair: "apr",
      williamhill: "Apr"
    },
    "maggio": {
      betfair: "mag",
      williamhill: "Mag"
    },
  }
}

export const typesNRoles = {
""
}
